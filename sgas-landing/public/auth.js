...
(function () {
  ...
  function isPublicDemo() {
    ...
    return projectId.indexOf('demo-') === 0;
  }
    ...
    try {
      ...
      if (typeof syncFromSupabase === 'function') {
        syncFromSupabase().then(function (ok) {
          ...
          // Auto-open wizard for first-time users with no readiness assessment
          setTimeout(function() {
            if (typeof window.openWizardIfNoAssessment === 'function') {
              window.openWizardIfNoAssessment();
            }
          }, 600);
        });
      }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // A demonstração explícita é pública e somente de leitura; os restantes
    // acessos exigem autenticação.
    if (isPublicDemo()) {
      window.SGAS_DEMO_ACTIVE = true;
      window.hideAuthOverlay();
      document.body.classList.add('sgas-demo-readonly');
      document.addEventListener('click', function (event) {
        var button = event.target && event.target.closest ? event.target.closest('button') : null;
        if (!button) return;
        var action = button.getAttribute('onclick') || '';
        if (!/(open.*modal|add|delete|remove|save|activate|checkout|create|submit|toggle|^del)/i.test(action)) return;
        event.preventDefault();
        event.stopImmediatePropagation();
        alert('A demonstração é apenas de leitura. Cria uma conta para guardar alterações.');
      }, true);
    } else {
      window.showAuthOverlay();
    }
    ...
    sb.auth.onAuthStateChange(async function (event, session) {
      ...
      if (event === 'SIGNED_OUT' && !isPublicDemo()) window.showAuthOverlay();
